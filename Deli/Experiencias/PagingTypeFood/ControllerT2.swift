...
class ControllerT2: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    
    var gridContainer: UICollectionView?
    var layout: UICollectionViewFlowLayout?
    var imgArray = [UIImage]()
    var dataSourceForMariscos3 = [Restaurant]()
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do{
                ...
                if json.count > 0{ //numero de post en el json
                    self.dataSourceForMariscos3.removeAll()
                    for obj in json{ //Recorrido de todos los post en el json
                        ...
                        if restaurante.tag.lowercased() == "mariscos" {
                            self.dataSourceForMariscos3.append(restaurante)
                        }
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return dataSourceForMariscos3.count
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let url = URL(string: "http://104.236.10.17" + dataSourceForMariscos3[indexPath.row].thumbnail) {
        ...
        //Zona de restaurante
        cell.restaurantZone.text = dataSourceForMariscos3[indexPath.row].zona
        ...
        cell.restaurantName.text = dataSourceForMariscos3[indexPath.row].nombre
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        details.showRestaurant = dataSourceForMariscos3[indexPath.row]
        details.imagen_principal = dataSourceForMariscos3[indexPath.row].imagen_principal
        details.tipo_comida = dataSourceForMariscos3[indexPath.row].tipo_comida
        details.name_restaurant = dataSourceForMariscos3[indexPath.row].nombre
        details.incluye = dataSourceForMariscos3[indexPath.row].incluye
        details.img_price = dataSourceForMariscos3[indexPath.row].img_price
        details.precio = dataSourceForMariscos3[indexPath.row].precio
        details.logo = dataSourceForMariscos3[indexPath.row].logo
        details.introduccion = dataSourceForMariscos3[indexPath.row].introduccion
        details.p1 = dataSourceForMariscos3[indexPath.row].p1
        details.link_video = dataSourceForMariscos3[indexPath.row].link_video
        details.p2 = dataSourceForMariscos3[indexPath.row].p2
        details.imagen_2 = dataSourceForMariscos3[indexPath.row].imagen2
        details.frase = dataSourceForMariscos3[indexPath.row].frase
        details.p3 = dataSourceForMariscos3[indexPath.row].p3
        details.imagen_3 = dataSourceForMariscos3[indexPath.row].imagen3
        details.logo_editorial = dataSourceForMariscos3[indexPath.row].logo_editorial
        details.editorial = dataSourceForMariscos3[indexPath.row].editorial
        details.id_restaurant = dataSourceForMariscos3[indexPath.row].id_restaurante
        details.category = dataSourceForMariscos3[indexPath.row].categoria
        details.zona = dataSourceForMariscos3[indexPath.row].zona
        details.direccion = dataSourceForMariscos3[indexPath.row].direccion
