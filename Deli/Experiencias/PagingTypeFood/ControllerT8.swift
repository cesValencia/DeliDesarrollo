...
class ControllerT8: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    
    var gridContainer: UICollectionView?
    var layout: UICollectionViewFlowLayout?
    var imgArray = [UIImage]()
    var dataSourceForSaludable3 = [Restaurant]()
    ...
    func loadUI() {
        ...
        gridContainer?.register(Content.self, forCellWithReuseIdentifier: "cell8")
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do{
                ...
                if json.count > 0{ //numero de post en el json
                    self.dataSourceForSaludable3.removeAll()
                    for obj in json{ //Recorrido de todos los post en el json
                        ...
                        if restaurante.tag.lowercased() == "saludable" {
                            
                            self.dataSourceForSaludable3.append(restaurante);
                        }
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return dataSourceForSaludable3.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell8", for: indexPath) as! Content
        
        if let url = URL(string: "http://104.236.10.17" + dataSourceForSaludable3[indexPath.row].thumbnail) {
            ...
            print(dataSourceForSaludable3[indexPath.row].thumbnail)
        } else {
            cell.restaurantImg.frame = CGRect(x: cell.frame.width * 0.075, y: cell.frame.height * 0.05, width: cell.frame.width * 0.85, height: cell.frame.height * 0.9)
        ...
        //Zona de restaurante
        cell.restaurantZone.text = dataSourceForSaludable3[indexPath.row].zona
        ...
        cell.restaurantName.text = dataSourceForSaludable3[indexPath.row].nombre
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        details.showRestaurant = dataSourceForSaludable3[indexPath.row]
        details.imagen_principal = dataSourceForSaludable3[indexPath.row].imagen_principal
        details.tipo_comida = dataSourceForSaludable3[indexPath.row].tipo_comida
        details.name_restaurant = dataSourceForSaludable3[indexPath.row].nombre
        details.incluye = dataSourceForSaludable3[indexPath.row].incluye
        details.img_price = dataSourceForSaludable3[indexPath.row].img_price
        details.precio = dataSourceForSaludable3[indexPath.row].precio
        details.logo = dataSourceForSaludable3[indexPath.row].logo
        details.introduccion = dataSourceForSaludable3[indexPath.row].introduccion
        details.p1 = dataSourceForSaludable3[indexPath.row].p1
        details.link_video = dataSourceForSaludable3[indexPath.row].link_video
        details.p2 = dataSourceForSaludable3[indexPath.row].p2
        details.imagen_2 = dataSourceForSaludable3[indexPath.row].imagen2
        details.frase = dataSourceForSaludable3[indexPath.row].frase
        details.p3 = dataSourceForSaludable3[indexPath.row].p3
        details.imagen_3 = dataSourceForSaludable3[indexPath.row].imagen3
        details.logo_editorial = dataSourceForSaludable3[indexPath.row].logo_editorial
        details.editorial = dataSourceForSaludable3[indexPath.row].editorial
        details.id_restaurant = dataSourceForSaludable3[indexPath.row].id_restaurante
        details.category = dataSourceForSaludable3[indexPath.row].categoria
        details.zona = dataSourceForSaludable3[indexPath.row].zona
        details.direccion = dataSourceForSaludable3[indexPath.row].direccion
