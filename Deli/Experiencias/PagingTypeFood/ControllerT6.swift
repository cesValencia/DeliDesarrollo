...
class ControllerT6: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    
    var gridContainer: UICollectionView?
    var layout: UICollectionViewFlowLayout?
    var imgArray = [UIImage]()
    var dataSourceForMexicana3 = [Restaurant]()
    ...
    func loadUI() {
        ...
        gridContainer?.register(Content.self, forCellWithReuseIdentifier: "cell6")
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do{
                ...
                if json.count > 0{ //numero de post en el json
                    self.dataSourceForMexicana3.removeAll()
                    for obj in json{ //Recorrido de todos los post en el json
                        ...
                        if restaurante.tag.lowercased() == "mexicana" {
                            self.dataSourceForMexicana3.append(restaurante);
                        }
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return dataSourceForMexicana3.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell6", for: indexPath) as! Content
        
        if let url = URL(string: "http://104.236.10.17" + dataSourceForMexicana3[indexPath.row].thumbnail) {
        ...
        //Zona de restaurante
        cell.restaurantZone.text = dataSourceForMexicana3[indexPath.row].zona
        ...
        cell.restaurantName.text = dataSourceForMexicana3[indexPath.row].nombre
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        details.showRestaurant = dataSourceForMexicana3[indexPath.row]
        details.imagen_principal = dataSourceForMexicana3[indexPath.row].imagen_principal
        details.tipo_comida = dataSourceForMexicana3[indexPath.row].tipo_comida
        details.name_restaurant = dataSourceForMexicana3[indexPath.row].nombre
        details.incluye = dataSourceForMexicana3[indexPath.row].incluye
        details.img_price = dataSourceForMexicana3[indexPath.row].img_price
        details.precio = dataSourceForMexicana3[indexPath.row].precio
        details.logo = dataSourceForMexicana3[indexPath.row].logo
        details.introduccion = dataSourceForMexicana3[indexPath.row].introduccion
        details.p1 = dataSourceForMexicana3[indexPath.row].p1
        details.link_video = dataSourceForMexicana3[indexPath.row].link_video
        details.p2 = dataSourceForMexicana3[indexPath.row].p2
        details.imagen_2 = dataSourceForMexicana3[indexPath.row].imagen2
        details.frase = dataSourceForMexicana3[indexPath.row].frase
        details.p3 = dataSourceForMexicana3[indexPath.row].p3
        details.imagen_3 = dataSourceForMexicana3[indexPath.row].imagen3
        details.logo_editorial = dataSourceForMexicana3[indexPath.row].logo_editorial
        details.editorial = dataSourceForMexicana3[indexPath.row].editorial
        details.id_restaurant = dataSourceForMexicana3[indexPath.row].id_restaurante
        details.category = dataSourceForMexicana3[indexPath.row].categoria
        details.zona = dataSourceForMexicana3[indexPath.row].zona
        details.direccion = dataSourceForMexicana3[indexPath.row].direccion
