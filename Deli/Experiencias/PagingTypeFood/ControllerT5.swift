...
class ControllerT5: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    
    var gridContainer: UICollectionView?
    var layout: UICollectionViewFlowLayout?
    var imgArray = [UIImage]()
    var dataSourceForOriental3 = [Restaurant]()
    ...
    func loadUI() {
        ...
        gridContainer?.register(Content.self, forCellWithReuseIdentifier: "cell5")
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do{
                ...
                if json.count > 0{ //numero de post en el json
                    self.dataSourceForOriental3.removeAll()
                    for obj in json{ //Recorrido de todos los post en el json
                        ...
                        if restaurante.tag.lowercased() == "oriental" {
                            self.dataSourceForOriental3.append(restaurante);
                        }
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return dataSourceForOriental3.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell5", for: indexPath) as! Content
        
        if let url = URL(string: "http://104.236.10.17" + dataSourceForOriental3[indexPath.row].thumbnail) {
        ...
        //Zona de restaurante
        cell.restaurantZone.text = dataSourceForOriental3[indexPath.row].zona
        ...
        cell.restaurantName.text = dataSourceForOriental3[indexPath.row].nombre
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        details.showRestaurant = dataSourceForOriental3[indexPath.row]
        details.imagen_principal = dataSourceForOriental3[indexPath.row].imagen_principal
        details.tipo_comida = dataSourceForOriental3[indexPath.row].tipo_comida
        details.name_restaurant = dataSourceForOriental3[indexPath.row].nombre
        details.incluye = dataSourceForOriental3[indexPath.row].incluye
        details.img_price = dataSourceForOriental3[indexPath.row].img_price
        details.precio = dataSourceForOriental3[indexPath.row].precio
        details.logo = dataSourceForOriental3[indexPath.row].logo
        details.introduccion = dataSourceForOriental3[indexPath.row].introduccion
        details.p1 = dataSourceForOriental3[indexPath.row].p1
        details.link_video = dataSourceForOriental3[indexPath.row].link_video
        details.p2 = dataSourceForOriental3[indexPath.row].p2
        details.imagen_2 = dataSourceForOriental3[indexPath.row].imagen2
        details.frase = dataSourceForOriental3[indexPath.row].frase
        details.p3 = dataSourceForOriental3[indexPath.row].p3
        details.imagen_3 = dataSourceForOriental3[indexPath.row].imagen3
        details.logo_editorial = dataSourceForOriental3[indexPath.row].logo_editorial
        details.editorial = dataSourceForOriental3[indexPath.row].editorial
        details.id_restaurant = dataSourceForOriental3[indexPath.row].id_restaurante
        details.category = dataSourceForOriental3[indexPath.row].categoria
        details.zona = dataSourceForOriental3[indexPath.row].zona
        details.direccion = dataSourceForOriental3[indexPath.row].direccion
