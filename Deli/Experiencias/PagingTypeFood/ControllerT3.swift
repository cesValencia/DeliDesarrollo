...
class ControllerT3: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    
    var gridContainer: UICollectionView?
    var layout: UICollectionViewFlowLayout?
    var imgArray = [UIImage]()
    var dataSourceForPastas3 = [Restaurant]()
    ...
    func loadUI() {
        ...
        gridContainer?.register(Content.self, forCellWithReuseIdentifier: "cell3")
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do{
                ...
                if json.count > 0{ //numero de post en el json
                    self.dataSourceForPastas3.removeAll()
                    for obj in json{ //Recorrido de todos los post en el json
                        ...
                        if restaurante.tag.lowercased() == "pastas" {
                            self.dataSourceForPastas3.append(restaurante);
                        }
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return dataSourceForPastas3.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell3", for: indexPath) as! Content
        
        if let url = URL(string: "http://104.236.10.17" + dataSourceForPastas3[indexPath.row].thumbnail) {
        ...
        //Zona de restaurante
        cell.restaurantZone.text = dataSourceForPastas3[indexPath.row].zona
        ...
        cell.restaurantName.text = dataSourceForPastas3[indexPath.row].nombre
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        details.showRestaurant = dataSourceForPastas3[indexPath.row]
        details.imagen_principal = dataSourceForPastas3[indexPath.row].imagen_principal
        details.tipo_comida = dataSourceForPastas3[indexPath.row].tipo_comida
        details.name_restaurant = dataSourceForPastas3[indexPath.row].nombre
        details.incluye = dataSourceForPastas3[indexPath.row].incluye
        details.img_price = dataSourceForPastas3[indexPath.row].img_price
        details.precio = dataSourceForPastas3[indexPath.row].precio
        details.logo = dataSourceForPastas3[indexPath.row].logo
        details.introduccion = dataSourceForPastas3[indexPath.row].introduccion
        details.p1 = dataSourceForPastas3[indexPath.row].p1
        details.link_video = dataSourceForPastas3[indexPath.row].link_video
        details.p2 = dataSourceForPastas3[indexPath.row].p2
        details.imagen_2 = dataSourceForPastas3[indexPath.row].imagen2
        details.frase = dataSourceForPastas3[indexPath.row].frase
        details.p3 = dataSourceForPastas3[indexPath.row].p3
        details.imagen_3 = dataSourceForPastas3[indexPath.row].imagen3
        details.logo_editorial = dataSourceForPastas3[indexPath.row].logo_editorial
        details.editorial = dataSourceForPastas3[indexPath.row].editorial
        details.id_restaurant = dataSourceForPastas3[indexPath.row].id_restaurante
        details.category = dataSourceForPastas3[indexPath.row].categoria
        details.zona = dataSourceForPastas3[indexPath.row].zona
        details.direccion = dataSourceForPastas3[indexPath.row].direccion
