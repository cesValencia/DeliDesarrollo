...
class ControllerT7: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    
    var gridContainer: UICollectionView?
    var layout: UICollectionViewFlowLayout?
    var imgArray = [UIImage]()
    var dataSourceForEuropea3 = [Restaurant]()
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do{
                ...
                if json.count > 0{ //numero de post en el json
                    self.dataSourceForEuropea3.removeAll()
                    for obj in json{ //Recorrido de todos los post en el json
                        ...
                        if restaurante.tag.lowercased() == "europea" {
                            self.dataSourceForEuropea3.append(restaurante);
                        }
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return dataSourceForEuropea3.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell7", for: indexPath) as! Content
        
        if let url = URL(string: "http://104.236.10.17" + dataSourceForEuropea3[indexPath.row].thumbnail) {
        ...
        //Zona de restaurante
        cell.restaurantZone.text = dataSourceForEuropea3[indexPath.row].zona
        ...
        cell.restaurantName.text = dataSourceForEuropea3[indexPath.row].nombre
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        details.showRestaurant = dataSourceForEuropea3[indexPath.row]
        details.imagen_principal = dataSourceForEuropea3[indexPath.row].imagen_principal
        details.tipo_comida = dataSourceForEuropea3[indexPath.row].tipo_comida
        details.name_restaurant = dataSourceForEuropea3[indexPath.row].nombre
        details.incluye = dataSourceForEuropea3[indexPath.row].incluye
        details.img_price = dataSourceForEuropea3[indexPath.row].img_price
        details.precio = dataSourceForEuropea3[indexPath.row].precio
        details.logo = dataSourceForEuropea3[indexPath.row].logo
        details.introduccion = dataSourceForEuropea3[indexPath.row].introduccion
        details.p1 = dataSourceForEuropea3[indexPath.row].p1
        details.link_video = dataSourceForEuropea3[indexPath.row].link_video
        details.p2 = dataSourceForEuropea3[indexPath.row].p2
        details.imagen_2 = dataSourceForEuropea3[indexPath.row].imagen2
        details.frase = dataSourceForEuropea3[indexPath.row].frase
        details.p3 = dataSourceForEuropea3[indexPath.row].p3
        details.imagen_3 = dataSourceForEuropea3[indexPath.row].imagen3
        details.logo_editorial = dataSourceForEuropea3[indexPath.row].logo_editorial
        details.editorial = dataSourceForEuropea3[indexPath.row].editorial
        details.id_restaurant = dataSourceForEuropea3[indexPath.row].id_restaurante
        details.category = dataSourceForEuropea3[indexPath.row].categoria
        details.zona = dataSourceForEuropea3[indexPath.row].zona
        details.direccion = dataSourceForEuropea3[indexPath.row].direccion
