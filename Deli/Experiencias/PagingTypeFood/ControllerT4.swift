...
class ControllerT4: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    
    var gridContainer: UICollectionView?
    var layout: UICollectionViewFlowLayout?
    var imgArray = [UIImage]()
    var dataSourceForSnack3 = [Restaurant]()
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do{
                ...
                if json.count > 0{ //numero de post en el json
                    self.dataSourceForSnack3.removeAll()
                    for obj in json{ //Recorrido de todos los post en el json
                        ...
                        if restaurante.tag.lowercased() == "snacks" {
                            self.dataSourceForSnack3.append(restaurante);
                        }
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return dataSourceForSnack3.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell4", for: indexPath) as! Content
        
        if let url = URL(string: "http://104.236.10.17" + dataSourceForSnack3[indexPath.row].thumbnail) {
        ...
        //Zona de restaurante
        cell.restaurantZone.text = dataSourceForSnack3[indexPath.row].zona
        ...
        cell.restaurantName.text = dataSourceForSnack3[indexPath.row].nombre
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        details.showRestaurant = dataSourceForSnack3[indexPath.row]
        details.imagen_principal = dataSourceForSnack3[indexPath.row].imagen_principal
        details.tipo_comida = dataSourceForSnack3[indexPath.row].tipo_comida
        details.name_restaurant = dataSourceForSnack3[indexPath.row].nombre
        details.incluye = dataSourceForSnack3[indexPath.row].incluye
        details.img_price = dataSourceForSnack3[indexPath.row].img_price
        details.precio = dataSourceForSnack3[indexPath.row].precio
        details.logo = dataSourceForSnack3[indexPath.row].logo
        details.introduccion = dataSourceForSnack3[indexPath.row].introduccion
        details.p1 = dataSourceForSnack3[indexPath.row].p1
        details.link_video = dataSourceForSnack3[indexPath.row].link_video
        details.p2 = dataSourceForSnack3[indexPath.row].p2
        details.imagen_2 = dataSourceForSnack3[indexPath.row].imagen2
        details.frase = dataSourceForSnack3[indexPath.row].frase
        details.p3 = dataSourceForSnack3[indexPath.row].p3
        details.imagen_3 = dataSourceForSnack3[indexPath.row].imagen3
        details.logo_editorial = dataSourceForSnack3[indexPath.row].logo_editorial
        details.editorial = dataSourceForSnack3[indexPath.row].editorial
        details.id_restaurant = dataSourceForSnack3[indexPath.row].id_restaurante
        details.category = dataSourceForSnack3[indexPath.row].categoria
        details.zona = dataSourceForSnack3[indexPath.row].zona
        details.direccion = dataSourceForSnack3[indexPath.row].direccion
