...
class Controller1: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
...
//        getData()
    ...
    func loadUI() {
        ...
        gridContainer?.register(Content.self, forCellWithReuseIdentifier: "cell1")
...
//                        self.dataSource.append(restaurante);
                        if restaurante.categoria.lowercased() == "diamante" {
                            self.dataSource.append(restaurante)
                        }
...
//                        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell1", for: indexPath) as! Content
        
        
        
        if let url = URL(string: "http://104.236.10.17" + dataSource[indexPath.row].thumbnail) {
            ...
            print(dataSource[indexPath.row].thumbnail)
        } else {
            cell.restaurantImg.frame = CGRect(x: cell.frame.width * 0.075, y: cell.frame.height * 0.05, width: cell.frame.width * 0.85, height: cell.frame.height * 0.9)
        ...
        //Zona de restaurante
        cell.restaurantZone.text = dataSource[indexPath.row].zona
        ...
        cell.restaurantName.text = dataSource[indexPath.row].nombre
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        details.showRestaurant = dataSource[indexPath.row]
        details.imagen_principal = dataSource[indexPath.row].imagen_principal
        details.tipo_comida = dataSource[indexPath.row].tipo_comida
        details.name_restaurant = dataSource[indexPath.row].nombre
        details.incluye = dataSource[indexPath.row].incluye
        details.img_price = dataSource[indexPath.row].img_price
        details.precio = dataSource[indexPath.row].precio
        details.logo = dataSource[indexPath.row].logo
        details.introduccion = dataSource[indexPath.row].introduccion
        details.p1 = dataSource[indexPath.row].p1
        details.link_video = dataSource[indexPath.row].link_video
        details.p2 = dataSource[indexPath.row].p2
        details.imagen_2 = dataSource[indexPath.row].imagen2
        details.frase = dataSource[indexPath.row].frase
        details.p3 = dataSource[indexPath.row].p3
        details.imagen_3 = dataSource[indexPath.row].imagen3
        details.logo_editorial = dataSource[indexPath.row].logo_editorial
        details.editorial = dataSource[indexPath.row].editorial
        details.id_restaurant = dataSource[indexPath.row].id_restaurante
        details.category = dataSource[indexPath.row].categoria
        details.zona = dataSource[indexPath.row].zona
        details.direccion = dataSource[indexPath.row].direccion
        details.tolerancia = dataSource[indexPath.row].tolerancia
        
//        present(details, animated: true, completion: nil)
       
        NSLog("presentando vc"); parentController?.navigationController?.pushViewController(details, animated: true)
        NSLog("acabo de presentarlo")
    }
