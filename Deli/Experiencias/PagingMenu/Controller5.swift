...
class Controller5: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    
    var gridContainer: UICollectionView?
    var layout: UICollectionViewFlowLayout?
    var imgArray = [UIImage]()
    var dataSourceForVerde = [Restaurant]()
...
//        getData()
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do{
                ...
                if json.count > 0{ //numero de post en el json
                    self.dataSourceForVerde.removeAll()
                    for obj in json{ //Recorrido de todos los post en el json
                        ...
                        if restaurante.categoria.lowercased() == "verdes" {
                            self.dataSourceForVerde.append(restaurante);
                        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        //Zona de restaurante
        cell.restaurantZone.text = dataSourceForVerde[indexPath.row].zona
        ...
        cell.restaurantName.text = dataSourceForVerde[indexPath.row].nombre
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        details.showRestaurant = dataSourceForVerde[indexPath.row]
        details.imagen_principal = dataSourceForVerde[indexPath.row].imagen_principal
        details.tipo_comida = dataSourceForVerde[indexPath.row].tipo_comida
        details.name_restaurant = dataSourceForVerde[indexPath.row].nombre
        details.incluye = dataSourceForVerde[indexPath.row].incluye
        details.img_price = dataSourceForVerde[indexPath.row].img_price
        details.precio = dataSourceForVerde[indexPath.row].precio
        details.logo = dataSourceForVerde[indexPath.row].logo
        details.introduccion = dataSourceForVerde[indexPath.row].introduccion
        details.p1 = dataSourceForVerde[indexPath.row].p1
        details.link_video = dataSourceForVerde[indexPath.row].link_video
        details.p2 = dataSourceForVerde[indexPath.row].p2
        details.imagen_2 = dataSourceForVerde[indexPath.row].imagen2
        details.frase = dataSourceForVerde[indexPath.row].frase
        details.p3 = dataSourceForVerde[indexPath.row].p3
        details.imagen_3 = dataSourceForVerde[indexPath.row].imagen3
        details.logo_editorial = dataSourceForVerde[indexPath.row].logo_editorial
        details.editorial = dataSourceForVerde[indexPath.row].editorial
        details.id_restaurant = dataSourceForVerde[indexPath.row].id_restaurante
        details.category = dataSourceForVerde[indexPath.row].categoria
        details.zona = dataSourceForVerde[indexPath.row].zona
        details.direccion = dataSourceForVerde[indexPath.row].direccion
        details.tolerancia = dataSourceForVerde[indexPath.row].tolerancia
