...
class Controller4: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    
    var gridContainer: UICollectionView?
    var layout: UICollectionViewFlowLayout?
    var imgArray = [UIImage]()
    var dataSourceForPlata = [Restaurant]()
...
//        getData()
    ...
    func loadUI() {
        ...
        gridContainer?.register(Content.self, forCellWithReuseIdentifier: "cell4")
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do{
                ...
                if json.count > 0{ //numero de post en el json
                    self.dataSourceForPlata.removeAll()
                    for obj in json{ //Recorrido de todos los post en el json
                        ...
                        if restaurante.categoria.lowercased() == "plata" {
                            self.dataSourceForPlata.append(restaurante);
                        }
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return dataSourceForPlata.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell4", for: indexPath) as! Content
        
        if let url = URL(string: "http://104.236.10.17" + dataSourceForPlata[indexPath.row].thumbnail) {
        ...
        //Zona de restaurante
        cell.restaurantZone.text = dataSourceForPlata[indexPath.row].zona
        ...
        cell.restaurantName.text = dataSourceForPlata[indexPath.row].nombre
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        details.showRestaurant = dataSourceForPlata[indexPath.row]
        details.imagen_principal = dataSourceForPlata[indexPath.row].imagen_principal
        details.tipo_comida = dataSourceForPlata[indexPath.row].tipo_comida
        details.name_restaurant = dataSourceForPlata[indexPath.row].nombre
        details.incluye = dataSourceForPlata[indexPath.row].incluye
        details.img_price = dataSourceForPlata[indexPath.row].img_price
        details.precio = dataSourceForPlata[indexPath.row].precio
        details.logo = dataSourceForPlata[indexPath.row].logo
        details.introduccion = dataSourceForPlata[indexPath.row].introduccion
        details.p1 = dataSourceForPlata[indexPath.row].p1
        details.link_video = dataSourceForPlata[indexPath.row].link_video
        details.p2 = dataSourceForPlata[indexPath.row].p2
        details.imagen_2 = dataSourceForPlata[indexPath.row].imagen2
        details.frase = dataSourceForPlata[indexPath.row].frase
        details.p3 = dataSourceForPlata[indexPath.row].p3
        details.imagen_3 = dataSourceForPlata[indexPath.row].imagen3
        details.logo_editorial = dataSourceForPlata[indexPath.row].logo_editorial
        details.editorial = dataSourceForPlata[indexPath.row].editorial
        details.id_restaurant = dataSourceForPlata[indexPath.row].id_restaurante
        details.category = dataSourceForPlata[indexPath.row].categoria
        details.zona = dataSourceForPlata[indexPath.row].zona
        details.direccion = dataSourceForPlata[indexPath.row].direccion
        details.tolerancia = dataSourceForPlata[indexPath.row].tolerancia
