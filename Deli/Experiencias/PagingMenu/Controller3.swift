...
class Controller3: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    
    var gridContainer: UICollectionView?
    var layout: UICollectionViewFlowLayout?
    var imgArray = [UIImage]()
    var dataSourceForMixo = [Restaurant]()
...
//        getData()
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do{
                ...
                if json.count > 0{ //numero de post en el json
                    self.dataSourceForMixo.removeAll()
                    for obj in json{ //Recorrido de todos los post en el json
                        ...
                        if restaurante.categoria.lowercased() == "mixologia" {
                            self.dataSourceForMixo.append(restaurante);
                        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        //Zona de restaurante
        cell.restaurantZone.text = dataSourceForMixo[indexPath.row].zona
        ...
        cell.restaurantName.text = dataSourceForMixo[indexPath.row].nombre
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        details.showRestaurant = dataSourceForMixo[indexPath.row]
        details.imagen_principal = dataSourceForMixo[indexPath.row].imagen_principal
        details.tipo_comida = dataSourceForMixo[indexPath.row].tipo_comida
        details.name_restaurant = dataSourceForMixo[indexPath.row].nombre
        details.incluye = dataSourceForMixo[indexPath.row].incluye
        details.img_price = dataSourceForMixo[indexPath.row].img_price
        details.precio = dataSourceForMixo[indexPath.row].precio
        details.logo = dataSourceForMixo[indexPath.row].logo
        details.introduccion = dataSourceForMixo[indexPath.row].introduccion
        details.p1 = dataSourceForMixo[indexPath.row].p1
        details.link_video = dataSourceForMixo[indexPath.row].link_video
        details.p2 = dataSourceForMixo[indexPath.row].p2
        details.imagen_2 = dataSourceForMixo[indexPath.row].imagen2
        details.frase = dataSourceForMixo[indexPath.row].frase
        details.p3 = dataSourceForMixo[indexPath.row].p3
        details.imagen_3 = dataSourceForMixo[indexPath.row].imagen3
        details.logo_editorial = dataSourceForMixo[indexPath.row].logo_editorial
        details.editorial = dataSourceForMixo[indexPath.row].editorial
        details.id_restaurant = dataSourceForMixo[indexPath.row].id_restaurante
        details.category = dataSourceForMixo[indexPath.row].categoria
        details.zona = dataSourceForMixo[indexPath.row].zona
        details.direccion = dataSourceForMixo[indexPath.row].direccion
        details.tolerancia = dataSourceForMixo[indexPath.row].tolerancia
