...
class Controller8: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    
    var gridContainer: UICollectionView?
    var layout: UICollectionViewFlowLayout?
    var imgArray = [UIImage]()
    var dataSourceForPostres = [Restaurant]()
...
//        getData()
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do{
                ...
                if json.count > 0{ //numero de post en el json
                    self.dataSourceForPostres.removeAll()
                    for obj in json{ //Recorrido de todos los post en el json
                        ...
                        if restaurante.categoria.lowercased() == "postres" {
                                
                                self.dataSourceForPostres.append(restaurante);
                        }
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return dataSourceForPostres.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell8", for: indexPath) as! Content
        
        if let url = URL(string: "http://104.236.10.17" + dataSourceForPostres[indexPath.row].thumbnail) {
            ...
            print(dataSourceForPostres[indexPath.row].thumbnail)
        } else {
            cell.restaurantImg.frame = CGRect(x: cell.frame.width * 0.075, y: cell.frame.height * 0.05, width: cell.frame.width * 0.85, height: cell.frame.height * 0.9)
        ...
        //Zona de restaurante
        cell.restaurantZone.text = dataSourceForPostres[indexPath.row].zona
        ...
        cell.restaurantName.text = dataSourceForPostres[indexPath.row].nombre
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        details.showRestaurant = dataSourceForPostres[indexPath.row]
        details.imagen_principal = dataSourceForPostres[indexPath.row].imagen_principal
        details.tipo_comida = dataSourceForPostres[indexPath.row].tipo_comida
        details.name_restaurant = dataSourceForPostres[indexPath.row].nombre
        details.incluye = dataSourceForPostres[indexPath.row].incluye
        details.img_price = dataSourceForPostres[indexPath.row].img_price
        details.precio = dataSourceForPostres[indexPath.row].precio
        details.logo = dataSourceForPostres[indexPath.row].logo
        details.introduccion = dataSourceForPostres[indexPath.row].introduccion
        details.p1 = dataSourceForPostres[indexPath.row].p1
        details.link_video = dataSourceForPostres[indexPath.row].link_video
        details.p2 = dataSourceForPostres[indexPath.row].p2
        details.imagen_2 = dataSourceForPostres[indexPath.row].imagen2
        details.frase = dataSourceForPostres[indexPath.row].frase
        details.p3 = dataSourceForPostres[indexPath.row].p3
        details.imagen_3 = dataSourceForPostres[indexPath.row].imagen3
        details.logo_editorial = dataSourceForPostres[indexPath.row].logo_editorial
        details.editorial = dataSourceForPostres[indexPath.row].editorial
        details.id_restaurant = dataSourceForPostres[indexPath.row].id_restaurante
        details.category = dataSourceForPostres[indexPath.row].categoria
        details.zona = dataSourceForPostres[indexPath.row].zona
        details.direccion = dataSourceForPostres[indexPath.row].direccion
        details.tolerancia = dataSourceForPostres[indexPath.row].tolerancia
