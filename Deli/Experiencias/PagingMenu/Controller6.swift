...
class Controller6: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    
    var gridContainer: UICollectionView?
    var layout: UICollectionViewFlowLayout?
    var imgArray = [UIImage]()
    var dataSourceForAntojo = [Restaurant]()
...
//        getData()
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do{
                ...
                if json.count > 0{ //numero de post en el json
                    self.dataSourceForAntojo.removeAll()
                    for obj in json{ //Recorrido de todos los post en el json
                        ...
                        if restaurante.categoria.lowercased() == "antojo" {
                            self.dataSourceForAntojo.append(restaurante);
                        }
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return dataSourceForAntojo.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell6", for: indexPath) as! Content

        if let url = URL(string: "http://104.236.10.17" + dataSourceForAntojo[indexPath.row].thumbnail) {
        ...
        //Zona de restaurante
        cell.restaurantZone.text = dataSourceForAntojo[indexPath.row].zona
        ...
        cell.restaurantName.text = dataSourceForAntojo[indexPath.row].nombre
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        details.showRestaurant = dataSourceForAntojo[indexPath.row]
        details.imagen_principal = dataSourceForAntojo[indexPath.row].imagen_principal
        details.tipo_comida = dataSourceForAntojo[indexPath.row].tipo_comida
        details.name_restaurant = dataSourceForAntojo[indexPath.row].nombre
        details.incluye = dataSourceForAntojo[indexPath.row].incluye
        details.img_price = dataSourceForAntojo[indexPath.row].img_price
        details.precio = dataSourceForAntojo[indexPath.row].precio
        details.logo = dataSourceForAntojo[indexPath.row].logo
        details.introduccion = dataSourceForAntojo[indexPath.row].introduccion
        details.p1 = dataSourceForAntojo[indexPath.row].p1
        details.link_video = dataSourceForAntojo[indexPath.row].link_video
        details.p2 = dataSourceForAntojo[indexPath.row].p2
        details.imagen_2 = dataSourceForAntojo[indexPath.row].imagen2
        details.frase = dataSourceForAntojo[indexPath.row].frase
        details.p3 = dataSourceForAntojo[indexPath.row].p3
        details.imagen_3 = dataSourceForAntojo[indexPath.row].imagen3
        details.logo_editorial = dataSourceForAntojo[indexPath.row].logo_editorial
        details.editorial = dataSourceForAntojo[indexPath.row].editorial
        details.id_restaurant = dataSourceForAntojo[indexPath.row].id_restaurante
        details.category = dataSourceForAntojo[indexPath.row].categoria
        details.zona = dataSourceForAntojo[indexPath.row].zona
        details.direccion = dataSourceForAntojo[indexPath.row].direccion
        details.tolerancia = dataSourceForAntojo[indexPath.row].tolerancia
